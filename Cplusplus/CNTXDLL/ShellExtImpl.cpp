...
//		_tcsreplace(szPath, _T('\\'), _T('/'));
        ...
        if(szPath[len-1] == _T('\\'))
            szPath[len-1] = _T('\0');
        //_tcsncat(m_szPath, _T(" --path=\""), cbPath); //path
		//_tcsncat(m_szPath, _T(" \""), cbPath); //path
		_tcsncat(m_szPath, _T(" \""), cbPath); //path
...
STDMETHODIMP CShellExt::QueryContextMenu(HMENU hMenu,UINT indexMenu,UINT idCmdFirst,UINT idCmdLast,UINT uFlags)
{
	...
	::InsertMenu(hMenu, indexMenu++, MF_STRING|MF_BYPOSITION, idCmd++, _T("Scan with GPM Antivirus"));
	// Seperator
	::InsertMenu(hMenu, indexMenu++, MF_SEPARATOR|MF_BYPOSITION, 0, NULL);
...
BOOL CShellExt::Scan(HWND hwnd)
{
    ...
    {
        MessageBox(hwnd, _T("Error: Unable to retrieve Path."), _T("GPM Antivirus"), MB_OK | MB_ICONERROR);
        return FALSE;
    }
    ...
    TCHAR szGPMPath[MAX_PATH] = _T("");
	TCHAR szScanner[MAX_PATH] = _T(""); //added
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\GPMAV"), 0, KEY_READ, &hKey))
    {
        cbData = sizeof(szGPMPath);
        RegQueryValueEx(hKey, _T("Install_Dir"), NULL, &dwType, (PBYTE)szGPMPath, &cbData);
        CloseHandle(hKey);
    }
    // try in hkey_local_machine if failed
    if (!_tcslen(szGPMPath) &&
    		(ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\GPMAV"), 0, KEY_READ, &hKey)))
    ...
    if(!_tcslen(szGPMPath))
    {
        ...
        {
            // get folder name
            _tsplitpath(szModule, NULL, szGPMPath, NULL, NULL);
        }
    }
    len = _tcslen(szGPMPath);
    if(!len)
    {
        MessageBox(hwnd, _T("Error: Unable to retrieve path to GPM. Please reinstall GPM Antivirus"), _T("GPM Antivirus"), MB_OK | MB_ICONERROR);
        delete [] szCmd;
        return FALSE;
    }
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\GPMAV"), 0, KEY_READ, &hKey))
    {
        cbData = sizeof(szScanner);
        RegQueryValueEx(hKey, _T("Scanner"), NULL, &dwType, (PBYTE)szScanner, &cbData);
        CloseHandle(hKey);
    }
    // try in hkey_local_machine if failed
    if (!_tcslen(szScanner) &&
    		(ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\GPMAV"), 0, KEY_READ, &hKey)))
    ...
    if(!_tcslen(szScanner))
    {
        ...
        {
            // get folder name
            _tsplitpath(szModule, NULL, szScanner, NULL, NULL);
        }
    }
    len = _tcslen(szScanner);
    if(!len)
    {
        MessageBox(hwnd, _T("Error: Unable to retrieve settings for Scanner. Please reconfigure GPM Antivirus"), _T("GPM Antivirus"), MB_OK | MB_ICONERROR);
        delete [] szCmd;
        return FALSE;
    }
	
    // Expand Env
    ExpandEnvironmentStrings(szGPMPath, szPathExpanded, sizeof(szPathExpanded));
    len = _tcslen(szPathExpanded);
    // remove trailing slash
    if(szPathExpanded[len-1] == _T('\\'))
        szPathExpanded[len-1] = _T('\0');
  //  _sntprintf(szCmd, szCmdSize, _T("\"%s\\%s\" --mode=scanner  %s"), //cmdline 1
 //_sntprintf(szCmd,  szCmdSize,  szScanner , //addded
   _sntprintf(szCmd, szCmdSize, _T("\"%s\\%s\" %s\\%s %s %s "), //cmdline 1
                szPathExpanded, _T("scanner.exe"), szPathExpanded, _T("gpmavscan.exe"), szScanner ,m_szPath);

				
    // read  optional params from registry
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER,
                _T("Classes\\CLSID\\{e76d8ce0-8944-4fed-b363-c2df08724673}\\InProcServer32"), 0, KEY_READ, &hKey) ||
    ...
    )
    {
        ...
        MessageBox(hwnd, szMsg, _T("GPM Antivirus"), MB_OK | MB_ICONERROR);
        delete [] szCmd;
        return FALSE;
    }
    delete [] szCmd;
...
STDMETHODIMP CShellExt::GetCommandString(UINT_PTR idCmd, UINT uType, UINT *pwReserved, LPSTR pszName, UINT cchMax)
{
	...
	case 0:
		if (GCS_HELPTEXTW == uType)
			wcsncpy((LPWSTR)pszName, L"GPM Antivirus", cchMax);
		else if (GCS_HELPTEXTA == uType)
			strncpy(pszName, "GPM Antivirus", cchMax);
		break;
   }
	return NOERROR;
}
